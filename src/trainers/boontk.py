...
class BooNTKTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for i in range(len(self.clients)):
            ...
            # Move all batch-level NTK tensors to CPU.
        ...
        # Init linear models
        theta_global = torch.zeros(2560, 200).cuda()
        ...
        np.save(loss_dir + '/loss_test' + self.dataset + self.model + '_5_communication_0.1lr', self.loss_list_test)  #
        np.save(acc_dir + '/acc_test' + self.dataset + self.model + '_5_communication_0.1lr', self.acc_list_test)
    ...
    def client_compute_eNTK(self,client_model, train_loader):
        ...
        return grads_data_list, target_list  # Do not return one-hot labels.
    ...
    def compute_eNTK(self, model, X, subsample_size=2560, seed=123):
        """"compute eNTK"""
        model.eval()
        ...
        # ====== Automatically compute total model parameter dimension without going out of bounds ======
        ...
        for i in tqdm(range(X.size()[0])):
            ...
            # ====== Force flattening consistently for CNN/MLP ======
            inp = X[i: i + 1]
