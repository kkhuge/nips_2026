...
class BaseTrainer(object):
    def __init__(self, options, dataset, another_dataset, model=None, optimizer=None, name='', worker=None):
        ...
        # # Iterate through each MiniDataset in the dictionary.
        # for i in self.all_train_data:
        #     dataset = self.all_train_data[i]  # Get the MiniDataset.
        ...
        # combined_features_train = torch.cat(combined_features_train, dim=0)  # Concatenate by rows.
        # combined_labels_train = torch.cat(combined_labels_train, dim=0)  # Concatenate by rows.
        ...
        #     dataset = self.all_test_data[i]  # Get the MiniDataset.
        #     features = torch.tensor(dataset.data)  # Extract features.
        #     labels = torch.tensor(dataset.labels)  # Extract labels.
        #
        ...
        #
        # # Merge all features and labels.
        # combined_features_test = torch.cat(combined_features_test, dim=0)  # Concatenate by rows.
        # combined_labels_test = torch.cat(combined_labels_test, dim=0)  # Concatenate by rows.
        ...
        # This automatically preserves transforms in the original datasets.
        self.all_train_data_combined = ConcatDataset(train_datasets_list)

        # Preserve a reference to the first client's data for compatibility with possible existing calls.
        if len(train_datasets_list) > 0:
            self.all_train_data_client_0 = train_datasets_list[0]

        # Preserve compatibility with the original logic: overwrite self.all_train_data with the merged dataset.
        ...
        # Merge the test set logically.
        self.all_test_data_combined = ConcatDataset(test_datasets_list)

        # Overwrite variables to keep compatibility.
    ...
    # Add the following method to the BaseTrainer class in base.py.
    def measure_gradient_heterogeneity(self, selected_clients):
        """
        Compute G^2 for the full model, G_b^2 for the backbone, and G_h^2 for the head.
        """
        # First ensure clients receive the latest global model parameters.
        ...
        # 2. Compute the weighted global average gradient.
        ...
        # 3. Compute variance, i.e. heterogeneity G_b^2 and G_h^2.
        var_b = 0.0
        var_h = 0.0
    ...
    def local_train_smoo(self, round_i, selected_clients, global_w, global_s, **kwargs):
        ...
        tilde_s_list = []
        for i, c in enumerate(selected_clients, start=1):
            # Ensure local initialization is bar{w}^{t}.
            c.set_flat_model_params(self.latest_model)

            # Local training obtains parameters, statistics, and perturbation factor tilde{s}_i to return globally.
    ...
    # Add this at the end of the BaseTrainer class in base.py:
    def local_train_gf(self, round_i, selected_clients, global_perturbed_model, c_interp, **kwargs):
        solns = []
        stats = []
        divergences = []  # Record each client's divergence || w^r - w_{i,K}^r ||_2.
        ...
        for i, c in enumerate(selected_clients, start=1):
            ...
            # Compute the current client's update divergence: || w^r - w_{i,K}^r ||_2.
            # soln is a tuple: (data_count, local_solution_tensor), so use soln[1].
