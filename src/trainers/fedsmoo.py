...
class FedSMOOTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for round_i in range(self.num_round):
            ...
            # Convert self.latest_model from flat shape to a tensor list for clients.
            global_w = []
            idx = 0
            ...
            # Run local FedSMOO training.
            solns, stats, tilde_s_list = self.local_train_smoo(round_i, selected_clients, global_w, self.global_s)

            self.metrics.extend_commu_stats(round_i, stats)

            # ======== FedSMOO global aggregation stage ========

            # 1. Update global perturbation s^{t+1} (Eq. 11: s = (1/n) sum_i tilde{s}_i).
            ...
            # Project bar_s into the ball with radius r, where r = self.rho.
            ...
            # 2. Compute this round's intermediate aggregated model (1/n) sum_i w_i^t.
            old_latest_model_flat = torch.cat([gw.flatten() for gw in global_w])
            self.latest_model = self.aggregate(solns)

            # ================= Key change: apply SAM and ADMM penalties only in stage one =================
            if round_i < 2000:

                # 3. Update global dual variable lambda^{t+1} (Algorithm 1 Line 18).
                ...
                m = len(self.clients)  # Total number of clients.
                ...
                # 4. Update global model w^{t+1} (Algorithm 1 Line 19).
