...
class FedPerWorker(LrdWorker):
    """
    FedPer Worker: separates base layers (WB) from personalized layers (WP) [cite: 9].
    """
    ...
    def get_base_params(self):
        """Extract base-layer parameters for global aggregation [cite: 43]."""
        params = []
    ...
    def set_base_params(self, flat_params):
        """Synchronize base-layer parameters from the server."""
        offset = 0
        for name, param in self.model.named_parameters():
            if "readout" not in name:
                numel = param.numel()
...
class FedPerTrainer(BaseTrainer):
    def __init__(self, options, dataset, another_dataset):
        ...
        # 1. Use a temporary model to get dimensions.
        ...
        # 4. Explicitly create independent personalized clients [cite: 42, 120].
    ...
    def train(self):
        ...
        for round_i in range(self.num_round):
            ...
            for c in selected_clients:
                # GPU memory management: move to GPU on demand.
                c.worker.model.to(self.device)

                # =================== Core fix [FIX] ===================
                # Corresponds to Algorithm 1: "Server sends current representation phi^t to these clients".
                # This step is required so clients receive the latest global Body before training the Head.
                c.worker.set_base_params(self.latest_model)
                # ====================================================

                # B. Local training: based on the latest phi^t, train the Head first, then update the Body [cite: 135, 142].
                (num_sample, _), stat = c.local_train(round_i)

                # C. Extract the WB update (Body) and send it back to the server [cite: 143].
            ...
            # 3. Aggregation: the server updates global Representation phi^{t+1} [cite: 143].
    ...
    def evaluate_personalized(self, round_i):
        ...
        for c in self.clients_per:
            ...
            # Testing also needs to synchronize the latest global Body to pair with the local Head [cite: 95].
            c.worker.set_base_params(self.latest_model)
            c.worker.model.eval()
