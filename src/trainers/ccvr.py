...
class CCVRTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        np.save(loss_dir + '/loss_test' + self.dataset + self.model + '_0.1lr', self.loss_list_test)  #
        np.save(acc_dir + '/acc_test' + self.dataset + self.model + '_0.1lr', self.acc_list_test)
    ...
    def ccvr_calibrate(self, global_model):
        """
        Memory-optimized version: the logic exactly matches your source code.
        """
        ...
        # ---- 1) client-side stats stored on CPU ----
        client_stats = []
        ...
        # There is only one aggregated result, so keeping it on GPU is fine, around 100 MB.
        Nc_list = torch.zeros(C, device=device)
        for stats_k in client_stats:
            for c in range(C):
                Nc_list[c] += stats_k[c]["Nc_k"].to(device)  # Move only when needed.
        ...
        for c in range(C):
            ...
            for stats_k in client_stats:
                ...
                # Dynamically move CPU data to GPU for computation, then release it.
        ...
        # GPU memory cleanup: client_stats is no longer needed.
        del client_stats
        import gc
        gc.collect()

        # ---- 3) sample virtual features, then store them on CPU ----
        virtual_Z = []
        virtual_Y = []
        for c in range(C):
            Nc = Nc_list[c].item()
        ...
        with torch.enable_grad():
            ...
            # # === Keep the loop exactly aligned with the original logic ===
    ...
    def compute_client_class_stats(self, global_model, loader, C, d, device):
        """
        Memory-optimized version: GPU computation -> CPU storage.
        Logic is unchanged; this only prevents client_stats from exhausting GPU memory.
        """
        # Keep accumulators on CPU.
        storage_device = 'cpu'
        ...
        for data, targets in loader:
            ...
            for c in unique_classes:
                ...
                # 2. Core computation on GPU.
                # Use matrix multiplication z.T @ z instead of an outer-product loop for speed and memory efficiency.
                batch_sum_x = z_c.sum(dim=0)
                batch_xxT = torch.mm(z_c.T, z_c)

                # 3. Move results to CPU immediately.
                c_idx = int(c.item())
                N_c[c_idx] += n_c_batch
        ...
        # Final statistic computation on CPU.
        stats_k = {}
        for c in range(C):
            n = N_c[c].item()
            ...
            # === Key point: return CPU tensors ===
            # This prevents appending to client_stats from using GPU memory.
            stats_k[c] = {
                "Nc_k": N_c[c],  # CPU
                "mu_c_k": mu_cpu,  # CPU
    ...
    def test_model(self, model, test_dataloader):
        ...
        with torch.no_grad():
            for x, y in test_dataloader:
                ...
                # 2. Key: apply the exact same preprocessing used during CCVR generation.
                if self.ccvr_apply_relu:
                    features = F.relu(features)
                if self.ccvr_apply_tukey:
                    features = torch.sign(features) * (torch.abs(features) ** self.ccvr_tukey_beta)

                # 3. Explicitly call the classifier layer instead of model(x).
