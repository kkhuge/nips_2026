...
class ScaffoldTrainer(BaseTrainer):
    ...
    def __init__(self, options, dataset, another_dataset):
        ...
        model_path = "scaffold_stage1_resnet18_cifar10_0.1_451.pth"
    ...
    def train(self):
        ...
        for round_i in range(0,100):
            ...
            #     torch.save(self.worker.model.state_dict(), "scaffold_stage1_resnet18_cifar10_0.1_450.pth")
            # # #     torch.save(self.worker.model.state_dict(), "scaffold_stage1_resnet18_cifar100_0.5.pth")
            # if round_i == 451:
            #     torch.save(self.worker.model.state_dict(), "scaffold_stage1_resnet18_cifar10_0.1_451.pth")
            #
            # if round_i == 452:
            #     torch.save(self.worker.model.state_dict(), "scaffold_stage1_resnet18_cifar10_0.1_452.pth")
            #
            # if round_i == 453:
            #     torch.save(self.worker.model.state_dict(), "scaffold_stage1_resnet18_cifar10_0.1_453.pth")
        ...
        np.save(loss_dir + '/loss_train' + self.dataset + self.model + '_451', self.loss_list_train)
        np.save(acc_dir + '/acc_train' + self.dataset + self.model + '_451', self.acc_list_train)
        np.save(loss_dir + '/loss_test' + self.dataset + self.model + '_451', self.loss_list_test)
        np.save(acc_dir + '/acc_test' + self.dataset + self.model + '_451', self.acc_list_test)
        #
        # np.save(loss_dir + '/loss_train' + self.dataset + self.model + '_freeze_451', self.loss_list_train)
        # np.save(acc_dir + '/acc_train' + self.dataset + self.model + '_freeze_451', self.acc_list_train)
        # np.save(loss_dir + '/loss_test' + self.dataset + self.model + '_freeze_451', self.loss_list_test)
        # np.save(acc_dir + '/acc_test' + self.dataset + self.model + '_freeze_451', self.acc_list_test)
    ...
    def aggregate(self, solns, delta_c):
        ...
        avg_delta_c = []
        for params in zip(*delta_c):  # Aggregate delta_c per layer.
            stacked = torch.stack(params)
            avg_delta_c.append(stacked.mean(dim=0))

        # ---- 3. Update global control variables ----
