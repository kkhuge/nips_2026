    ...
    def __init__(self, input_shape, out_dim):
        ...
        for m in self.modules():
            if isinstance(m, nn.Linear):
                m.weight.data.normal_(0, np.sqrt(1.5 / k)) # Initialize weights with N(0, 2/width). 100/4096
                if m.bias is not None:
                    m.bias.data.normal_(0, 0.1)  # Initialize biases with N(0, 0.1).
...
class TwoHiddenLayerFcCifar10(nn.Module):
    ...
    def forward(self, x):
        # x = x.view(-1, 32*32) # For CIFAR-10 binary classification.
        out = F.relu(self.fc1(x))
        out = self.fc2(out)
        return out
...
def choose_model(options):
    ...
    # ========= FedETF-specific models =========
        ...
        # The FedETF ResNet18 variant lives in a separate file.
    ...
    # ========= FedUV-specific models =========
        ...
        # The FedUV ResNet18 variant lives in resnet_uv.py.
        mod = importlib.import_module('src.models.resnet_uv')
        resnet_model = getattr(mod, 'resnet18_uv')  # Keep the name casing consistent.
        return resnet_model(num_classes=options['num_class'])

    elif model_name.startswith('resnet50'):
        # The ResNet50 variant lives in resnet_50.py.
        mod = importlib.import_module('src.models.resnet_50')
        resnet_model = getattr(mod, 'resnet50')  # Keep the name casing consistent.
        return resnet_model(num_classes=options['num_class'])

    # ========= Standard ResNet model family =========
