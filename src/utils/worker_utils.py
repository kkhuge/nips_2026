...
class TinyImageNetDataset_train(Dataset):
    def __init__(self, data, labels):
        ...
        # Change 1: data is now a list of paths (List[str]) and cannot be converted to uint8.
        # Save the list directly, or convert it to a NumPy array without specifying dtype.
    ...
    def __getitem__(self, index):
        # Change 2: read images from paths.
        path = self.image_paths[index]

        # Must convert to RGB because some images may be grayscale (L) or include alpha (RGBA).
        # Without conversion, later transforms will fail.
        try:
            img = Image.open(path).convert('RGB')
        except Exception as e:
            print(f"Error loading image: {path}, {e}")
            # If image reading fails, return an all-black image to prevent training interruption.
...
class TinyImageNetDataset_test(Dataset):
    def __init__(self, data, labels):
        ...
        # Change 1: also store only paths.
        self.image_paths = data
        self.labels = np.array(labels).astype("int64")
