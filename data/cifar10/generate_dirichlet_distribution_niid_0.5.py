...
def main():
    ...
    for user in range(NUM_USER):
        for i in range(10):
            ...
            # num_test = np.round(distribution[i][user] * (1000-10)).astype(int)
            # for s in range(num_test):
            #     test_X[user].append(cifar_testdata[i][s].tolist())
            # cifar_testdata[i] = cifar_testdata[i][num_test:]
            # test_y[user].extend(np.ones(num_test) * i)

    # Setup directory for train/test data
    print('>>> Set data path for CIFAR-10.')
    image = 1 if IMAGE_DATA else 0
    train_path = '{}/data/train/all_data_{}_dirichlet_niid_0.5.pkl'.format(cpath, image)
    test_path = '{}/data/test/all_data_{}_dirichlet_niid_0.5.pkl'.format(cpath, image)
